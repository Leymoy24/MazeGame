...
# ----------------№7.2-------------------------
# Создаем начальную строку с определением графа
print('digraph GameWorld {')

# Создаем узел графа для каждой комнаты
for room in rooms:
    # Добавляем узел графа с меткой
    print(f'\t{room.label}')

    # Для каждого действия в комнате
    for action, label in room.actions.items():
        # Получаем метку комнаты, в которую ведет переход
        to_label = label[-2:]
        # Добавляем ребро графа для перехода из текущей комнаты в следующую
        print(f'\t{room.label} -> {to_label}')

# Завершаем определение графа
print('}')
# ---------------------------------------------
